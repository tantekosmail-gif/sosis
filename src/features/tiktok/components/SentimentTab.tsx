...
import NegativeHighlightCard from "@/components/tiktok/NegativeHighlightCard";
import TopHashtags from "@/components/tiktok/TopHashtags";
...
import { useRecentTikTokSearches } from "../hooks/useRecentSearches";
...
export default function TikTokSentimentTab() {
  ...
  const { recent, addRecentSearch } = useRecentTikTokSearches();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        {SHOW_DISCOVER_PANEL && <TikTokDiscoverPanel onSelectAccount={handleSelectAccount} />}
      </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 dark:border-slate-800 pt-3">
            ...
            {recent.map((username) => (
              <button
                ...
                className="rounded-lg bg-slate-100 dark:bg-slate-800 px-2.5 py-1 text-xs font-medium text-slate-600 dark:text-slate-300 transition hover:bg-rose-50 hover:text-rose-700 dark:hover:bg-rose-950/40 dark:hover:text-rose-300"
              >
                @{username}
              </button>
            ))}
      ...
      {!loading && !error && data && (
        <>
          ...
          <TikTokProfileCard pageInfo={data.page_info} stats={data.stats} sentiment={data.sentiment} items={data.items} />
