...
import type {
  TikTokComment,
  TikTokPostItem,
  TikTokPostsSearchData,
  TikTokSentimentOverview,
...
interface RawTikTokPostsSearchData {
  query: string | null;
  total: number;
  limit: number;
  offset: number;
...
function normalizeSearchItem(raw: RawSearchItem): TikTokPostItem {
  ...
  const comments: TikTokComment[] = (raw.comments ?? []).map((comment) => ({
...
function aggregateSentiment(items: TikTokPostItem[]): TikTokSentimentOverview {
...
export async function searchTikTokPosts(params: {
  ...
  limit?: number;
  offset?: number;
}): Promise<TikTokPostsSearchData> {
  const { data } = await api.get<{ data: RawTikTokPostsSearchData }>("/api/v1/tiktok/posts/search", {
    params: {
      q: params.q,
