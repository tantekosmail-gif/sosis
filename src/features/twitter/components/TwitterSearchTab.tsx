...
import { useRecentTwitterSearches } from "@/features/twitter/hooks/useRecentSearches";
import { useTwitterMetadataSearch, type TwitterMetadataSort } from "@/features/twitter/hooks/useTwitterMetadataSearch";
import TwitterDetailPanel from "@/components/twitter/TwitterDetailPanel";
import { useInfiniteScroll } from "@/hooks/useInfiniteScroll";
import { useTranslation } from "@/lib/i18n/LanguageProvider";
import type { TwitterMetadataItem } from "@/features/twitter/services/metadata.service";

const SORT_OPTIONS: { key: TwitterMetadataSort; label: string }[] = [
  { key: "relevance", label: "Most Relevant" },
  { key: "newest", label: "Newest" },
  { key: "popular", label: "Most Popular" },
];

export interface TwitterSearchTabHandle {
  search: (keyword: string) => void;
  searchKeywords: (keywords: string[]) => void;
}

const TwitterSearchTab = forwardRef<TwitterSearchTabHandle>(function TwitterSearchTab(_props, ref) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-semibold text-slate-900 dark:text-slate-100">{t.twitterSearchTab.title}</h2>
      ...
      {loading && (
        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm shadow-sm dark:border-slate-700 dark:bg-slate-900">
          ...
          <span className="text-slate-500 dark:text-slate-400">{t.twitterSearchTab.loadingDesc}</span>
        </div>
      )}
      ...
      {!loading && items !== null && (
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[380px_minmax(0,1fr)] xl:items-start">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                  {t.twitterSearchTab.sortLabel}
                </label>
                <select
                  value={sortBy}
                  onChange={(e) => changeSort(e.target.value as TwitterMetadataSort)}
          ...
          <TwitterDetailPanel detail={postDetail} loading={detailLoading} error={detailError} />
        </div>
      )}
      ...
      {!loading && items === null && (
        <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center dark:border-slate-700 dark:bg-slate-900">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500">{t.twitterSearchTab.emptyTitle}</p>
        </div>
      )}
    </div>
  );
...
function TwitterMetadataRow({
  item,
  isSelected,
  onSelect,
}: {
  item: TwitterMetadataItem;
  isSelected: boolean;
  onSelect: () => void;
}) {
  ...
  return (
    ...
    >
      <button type="button" onClick={onSelect} className="min-w-0 flex-1 text-left">
        ...
        <p className="mt-0.5 truncate text-xs text-slate-500 dark:text-slate-400">@{item.username} · {item.author}</p>
        ...
        <div className="mt-1 flex items-center gap-2.5 text-[11px] font-semibold text-slate-700 dark:text-slate-300">
          ...
          {item.saved_replies_count > 0 && (
            <span className="flex items-center gap-1">
              <MessageCircle size={11} className="text-slate-400 dark:text-slate-500" />
              {formatCompact(item.saved_replies_count ?? 0)}
            </span>
          )}
...
TwitterSearchTab.displayName = "TwitterSearchTab";

export default TwitterSearchTab;
