...
import {
  ...
  getInstagramMetadataDetail,
  type InstagramMetadataDetail,
  type InstagramMetadataItem,
  type InstagramMetadataParams,
} from "../services/metadata.service";

export type { InstagramMetadataItem, InstagramMetadataDetail };

export type InstagramMetadataSort = "relevance" | "newest" | "popular";

const SORT_PARAMS: Record<InstagramMetadataSort, Pick<InstagramMetadataParams, "sortBy" | "order">> = {
...
function sortMergedItems(items: InstagramMetadataItem[], sort: InstagramMetadataSort): InstagramMetadataItem[] {
  const sorted = [...items];
  if (sort === "newest") {
...
async function fetchMergedPage(
  keywords: string[],
  sort: InstagramMetadataSort,
  page: number,
  pageSize: number,
): Promise<{ items: InstagramMetadataItem[]; total: number; totalPages: number }> {
  const pages = await Promise.all(
    keywords.map((kw) => getInstagramMetadata({ search: kw, page, pageSize, ...SORT_PARAMS[sort] })),
  );

  const merged = new Map<string, InstagramMetadataItem>();
  let total = 0;
  let totalPages = 1;
  pages.forEach((data) => {
...
export function useInstagramMetadataSearch() {
  const [keyword, setKeyword] = useState("");
  const [sortBy, setSortBy] = useState<InstagramMetadataSort>("newest");
  const [items, setItems] = useState<InstagramMetadataItem[] | null>(null);
  ...
  const [allItems, setAllItems] = useState<InstagramMetadataItem[] | null>(null);
  const [loadingAll, setLoadingAll] = useState(false);

  const [selectedPostId, setSelectedPostId] = useState<string | null>(null);
  const [postDetail, setPostDetail] = useState<InstagramMetadataDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState("");

  const activeQuery = useRef<{ keywords: string[]; sortBy: InstagramMetadataSort }>({ keywords: [], sortBy: "newest" });

  const fetchPage = useCallback(async (keywords: string[], sort: InstagramMetadataSort, targetPage: number) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Gagal mencari post Instagram");
      setItems(null);
      setTotal(0);
      setPage(1);
      setTotalPages(1);
  ...
  const openPostDetail = useCallback(async (id: string) => {
    ...
    try {
      setPostDetail(await getInstagramMetadataDetail(id));
    } catch (err) {
