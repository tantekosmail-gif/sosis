...
import { Loader2, Search, ExternalLink, Flame, ThumbsUp, MessageCircle } from "lucide-react";
import { toast } from "sonner";

import InstagramDetailPanel from "@/components/instagram/InstagramDetailPanel";
...
import { useRecentInstagramSearches } from "@/features/instagram/hooks/useRecentSearches";
import { useInstagramMetadataSearch, type InstagramMetadataSort } from "@/features/instagram/hooks/useInstagramMetadataSearch";
import { useInfiniteScroll } from "@/hooks/useInfiniteScroll";
import { useTranslation } from "@/lib/i18n/LanguageProvider";
import type { InstagramMetadataItem } from "@/features/instagram/services/metadata.service";

const SORT_OPTIONS: { key: InstagramMetadataSort; label: string }[] = [
  { key: "relevance", label: "Most Relevant" },
  { key: "newest", label: "Newest" },
  { key: "popular", label: "Most Popular" },
];

export interface InstagramSearchTabHandle {
  search: (keyword: string) => void;
  searchKeywords: (keywords: string[]) => void;
}

const InstagramSearchTab = forwardRef<InstagramSearchTabHandle>(function InstagramSearchTab(_props, ref) {
  const { t } = useTranslation();
  const [query, setQuery] = useState("");
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [activeSuggestionIndex, setActiveSuggestionIndex] = useState(-1);
  const [sortBy, setSortBy] = useState<InstagramMetadataSort>("newest");
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-semibold text-slate-900 dark:text-slate-100">{t.instagramSearchTab.title}</h2>
      ...
      {loading && (
        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm shadow-sm dark:border-slate-700 dark:bg-slate-900">
          ...
          <span className="text-slate-500 dark:text-slate-400">{t.instagramSearchTab.loadingDesc}</span>
        </div>
      )}
      ...
      {!loading && items !== null && (
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[380px_minmax(0,1fr)] xl:items-start">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                  {t.instagramSearchTab.sortLabel}
                </label>
                <select
                  value={sortBy}
                  onChange={(e) => changeSort(e.target.value as InstagramMetadataSort)}
          ...
          <InstagramDetailPanel detail={postDetail} loading={detailLoading} error={detailError} />
        </div>
      )}
      ...
      {!loading && items === null && (
        <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center dark:border-slate-700 dark:bg-slate-900">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500">{t.instagramSearchTab.emptyTitle}</p>
        </div>
      )}
    </div>
  );
...
function InstagramMetadataRow({
  item,
  isSelected,
  onSelect,
}: {
  item: InstagramMetadataItem;
  isSelected: boolean;
  onSelect: () => void;
}) {
  ...
  return (
    ...
    >
      <button type="button" onClick={onSelect} className="min-w-0 flex-1 text-left">
        ...
        >
          {item.title || item.content || "(no caption)"}
        </h3>
        <p className="mt-0.5 truncate text-xs text-slate-500 dark:text-slate-400">{item.author}</p>
        ...
        <div className="mt-1 flex items-center gap-2.5 text-[11px] font-semibold text-slate-700 dark:text-slate-300">
          ...
          {item.saved_comment_count > 0 && (
            <span className="flex items-center gap-1">
              ...
              {item.saved_comment_count.toLocaleString()}
            </span>
          )}
...
InstagramSearchTab.displayName = "InstagramSearchTab";

export default InstagramSearchTab;
