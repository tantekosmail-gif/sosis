  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center gap-3 border-b border-slate-100 px-6 py-4">
        ...
        <div>
          <h2 className="font-semibold text-slate-900">Sentimen Publik Gabungan</h2>
          <p className="text-xs text-slate-400">Agregat sentimen dari {platformCount} platform yang dipantau</p>
        </div>
      </div>

      <div className="p-6">
        {total === 0 ? (
          <p className="text-sm text-slate-400">Belum ada data sentimen yang dianalisis</p>
        ) : (
          <>
            <div className="mb-2 flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Sentimen Keseluruhan</span>
              <span className="text-[11px] text-slate-400">{total.toLocaleString("id-ID")} item dianalisis</span>
            </div>
