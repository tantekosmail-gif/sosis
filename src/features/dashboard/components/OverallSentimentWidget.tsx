  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center gap-3 border-b border-slate-100 px-6 py-4 dark:border-slate-800">
        ...
        <div>
          <h2 className="font-semibold text-slate-900 dark:text-slate-100">{t.overviewWidgets.overallSentiment.title}</h2>
          <p className="text-xs text-slate-400 dark:text-slate-500">
            {t.overviewWidgets.overallSentiment.descLine.replace("{n}", String(platformCount))}
          </p>
        </div>
      </div>
      ...
      <div className="p-6">
        {total === 0 ? (
          <p className="text-sm text-slate-400 dark:text-slate-500">{t.overviewWidgets.overallSentiment.empty}</p>
        ) : (
          <>
            <div className="mb-2 flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                {t.overviewWidgets.overallSentiment.overallLabel}
              </span>
              <span className="text-[11px] text-slate-400 dark:text-slate-500">
                {t.overviewWidgets.overallSentiment.itemsAnalyzed.replace("{n}", total.toLocaleString("id-ID"))}
              </span>
            </div>
