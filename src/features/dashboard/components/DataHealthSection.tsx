  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm p-5">
      <div className="mb-1 flex items-center gap-2">
        ...
        <h2 className="font-semibold text-slate-900 dark:text-slate-100">Kesehatan Data</h2>
      </div>
      <p className="mb-4 text-xs text-slate-400 dark:text-slate-500">
        Seberapa banyak data yang sudah selesai dianalisis per platform — hasil bisa parsial kalau belum 100%.
      </p>
          ...
          return (
            <div key={item.platform} className="rounded-xl border border-slate-100 dark:border-slate-800 p-3.5">
              ...
              <p className="mt-1.5 text-[11px] text-slate-400 dark:text-slate-500">
                {item.totalAnalyzed.toLocaleString("id-ID")} / {item.totalItems.toLocaleString("id-ID")} dianalisis ({pct.toFixed(0)}%)
              </p>
            </div>
          );
        })}
