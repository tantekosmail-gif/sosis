...
import { useRecentThreadsSearches } from "@/features/threads/hooks/useRecentSearches";
import { useThreadsMetadataSearch, type ThreadsMetadataSort } from "@/features/threads/hooks/useThreadsMetadataSearch";
import { useInfiniteScroll } from "@/hooks/useInfiniteScroll";
import { useTranslation } from "@/lib/i18n/LanguageProvider";
import type { ThreadsMetadataItem } from "@/features/threads/services/metadata.service";
import ThreadsDetailPanel from "@/components/threads/ThreadsDetailPanel";

const SORT_OPTIONS: { key: ThreadsMetadataSort; label: string }[] = [
  { key: "relevance", label: "Most Relevant" },
  { key: "newest", label: "Newest" },
  { key: "popular", label: "Most Popular" },
];

export interface ThreadsSearchTabHandle {
  search: (keyword: string) => void;
  searchKeywords: (keywords: string[]) => void;
}

const ThreadsSearchTab = forwardRef<ThreadsSearchTabHandle>(function ThreadsSearchTab(_props, ref) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-semibold text-slate-900 dark:text-slate-100">{t.threadsSearchTab.title}</h2>
      ...
      {loading && (
        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm shadow-sm dark:border-slate-700 dark:bg-slate-900">
          ...
          <span className="text-slate-500 dark:text-slate-400">{t.threadsSearchTab.loadingDesc}</span>
        </div>
      )}
      ...
      {!loading && items !== null && (
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[380px_minmax(0,1fr)] xl:items-start">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                  {t.threadsSearchTab.sortLabel}
                </label>
                <select
                  value={sortBy}
                  onChange={(e) => changeSort(e.target.value as ThreadsMetadataSort)}
          ...
          <ThreadsDetailPanel detail={postDetail} loading={detailLoading} error={detailError} />
        </div>
      )}
      ...
      {!loading && items === null && (
        <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center dark:border-slate-700 dark:bg-slate-900">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500">{t.threadsSearchTab.emptyTitle}</p>
        </div>
      )}
    </div>
  );
...
function ThreadsMetadataRow({
  item,
  isSelected,
  onSelect,
}: {
  item: ThreadsMetadataItem;
  isSelected: boolean;
  onSelect: () => void;
}) {
...
ThreadsSearchTab.displayName = "ThreadsSearchTab";

export default ThreadsSearchTab;
