...
import { Flame, History, Loader2, Search, Tag } from "lucide-react";
...
import InstagramSearchTab, { type InstagramSearchTabHandle } from "@/features/instagram/components/InstagramSearchTab";
...
const TABS = [
  { key: "terkini", label: "Analisis Post" },
...
export default function InstagramPage() {
  ...
  const searchRef = useRef<InstagramSearchTabHandle>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const trySearch = (attempt = 0) => {
        if (searchRef.current?.searchKeywords) {
          searchRef.current.searchKeywords(searchTerms);
        } else if (attempt < 20) {
  ...
  useEffect(() => {
    ...
    setTopic(first.id, first.topic);
  }, [keywordRecommendations, globalTopicId, setTopic]);
          ...
          >
            <div className="flex flex-col gap-3 sm:flex-row">
              <div className="relative flex-1">
                ...
                <input
                  placeholder={t.instagramSearchTab.keywordPlaceholder}
                  value={query}
                  onChange={(e) => {
              ...
              >
                <Search size={15} />
                {t.instagramSearchTab.searchButton}
              </button>
            </div>
          </form>
        )}

        {tab === "terkini" && <InstagramSearchTab ref={searchRef} />}
        {tab === "trending" && <InstagramTrendingTab />}
        {tab === "sentiment" && <InstagramSentimentTab />}
      </div>
    </DashboardLayout>
  );
}
