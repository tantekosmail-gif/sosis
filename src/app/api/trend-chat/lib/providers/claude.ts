...
export async function runClaude(
  ...
  // web_search (varian _20260209) jalan lewat code-execution container — kalau ada
  // pending tool use dari situ, request lanjutan WAJIB kirim balik container id yang
  // sama, kalau tidak API menolak dengan "container_id is required...".
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    if (response.stop_reason === "refusal") {
      controller.enqueue(
        encodeEvent({
          type: "error",
          message: "Claude menolak permintaan ini (safety refusal).",
        }),
      );
      return;
    }
    ...
    if (response.stop_reason === "pause_turn") {
      controller.enqueue(
        encodeEvent({ type: "status", message: "Melanjutkan pencarian..." }),
      );
      continue;
    }
    ...
    if (customToolUses.length === 0) {
      ...
      controller.enqueue(
        encodeEvent({ type: "answer", text: finalText?.text ?? "" }),
      );
      return;
    }
  ...
  controller.enqueue(
    encodeEvent({
      type: "error",
      message: "Terlalu banyak iterasi, dihentikan.",
    }),
  );
}
