...
function formatBucket(bucket: string, interval: string) {
  try {
    return format(new Date(bucket), interval === "day" ? "d MMM" : "HH:mm", { locale: idLocale });
  } catch {
    return bucket;
  }
}
...
function CustomTooltip({ active, payload, label, interval }: any) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-3 shadow-lg text-xs">
      <p className="mb-2 font-semibold text-slate-700 dark:text-slate-300">{formatBucket(label, interval)}</p>
      {payload
        .slice()
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
      <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 px-6 py-5">
        ...
        <div>
          <h2 className="font-semibold text-slate-900 dark:text-slate-100">Timeline Mentions per Jam</h2>
          <p className="text-xs text-slate-400 dark:text-slate-500">
            Mentions per {interval === "day" ? "hari" : "jam"} &middot; {keywords.length} keyword teratas
          </p>
        </div>
      </div>
