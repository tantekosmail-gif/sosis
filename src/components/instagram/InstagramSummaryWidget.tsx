...
import type { InstagramAccountSummary, InstagramAnalysisSummary } from "@/features/instagram/types/summary.types";
...
function AccountAvatar({ username }: { username: string }) {
  return (
    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-950/50 text-sm font-bold text-indigo-700 dark:text-indigo-300">
      {username.charAt(0).toUpperCase()}
    </span>
  );
}

function AccountSentimentBar({ sentiment }: { sentiment: InstagramAccountSummary["sentiment"] }) {
...
function downloadAccountsCsv(rows: InstagramAccountSummary[]) {
  const header = ["Username", "Posts", "Komentar", "Positif (%)", "Netral (%)", "Negatif (%)"];
  ...
  a.href = url;
  a.download = "analisis-instagram-per-akun.csv";
  a.click();
  URL.revokeObjectURL(url);
}

export default function InstagramSummaryWidget({
  data,
  onSelectAccount,
}: {
  data: InstagramAnalysisSummary;
