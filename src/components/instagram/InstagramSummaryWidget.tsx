...
import { AtSign, BarChart3, MessageSquare, PieChart } from "lucide-react";

import type { InstagramAnalysisSummary } from "@/features/instagram/types/summary.types";
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
      <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 px-6 py-4">
        ...
        <div>
          <h2 className="font-semibold text-slate-900 dark:text-slate-100">Ringkasan Analisis Instagram</h2>
      ...
      <div className="space-y-5 p-6">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatCard icon={AtSign} label="Total Post" value={overall.total_posts.toLocaleString("id-ID")} />
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {per_account.map((account) => (
                    <tr
                      ...
                      className={onSelectAccount ? "cursor-pointer hover:bg-indigo-50/60" : ""}
                    >
