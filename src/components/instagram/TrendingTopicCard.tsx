...
import { Activity, AtSign, ExternalLink, Hash, MessageCircle, ThumbsUp } from "lucide-react";

import type { InstagramTrendingPost, InstagramTrendingTopic } from "@/features/instagram/types/trending.types";
...
function getDominantSentiment(sentiment: InstagramTrendingTopic["sentiment"]) {
  ...
  return (
    <div
      className={`flex gap-3 rounded-xl border p-3 transition-colors ${
        isSelected ? "border-indigo-400 bg-indigo-50/40" : "border-transparent bg-slate-50 dark:bg-slate-950"
      }`}
    >
      <a href={post.url} target="_blank" rel="noopener noreferrer" className="block h-16 w-16 shrink-0">
        <FallbackImage src={post.thumbnail} className="h-16 w-16 rounded-lg" />
      </a>
      ...
      <div className="min-w-0 flex-1">
        <a
          ...
          className="flex items-start gap-1 text-xs font-medium leading-snug text-slate-700 dark:text-slate-300 hover:text-indigo-600"
        >
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-2 px-5 pb-4">
        <StatBox label="Jumlah Post" value={String(topic.posts.length)} />
        <StatBox
          label="Sentimen"
      ...
      <div className="flex-1 border-t border-slate-100 dark:border-slate-800 px-5 py-4">
        ...
        {topic.posts.length === 0 ? (
          ...
          <div className="space-y-3">
            ...
            {extraCount > 0 && (
              <p className="text-[11px] text-slate-400 dark:text-slate-500">+{extraCount} postingan lainnya</p>
            )}
          </div>
        )}
      </div>
