...
import { ChevronLeft, ChevronRight, Eye, Flame, MessageCircle, Play, RefreshCw } from "lucide-react";
        ...
        <div className="group relative">
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5">
              <p className="truncate text-[11px] font-medium uppercase tracking-wide text-white/70">{decodeHtmlEntities(active.channel)}</p>
              <p className="mt-1.5 line-clamp-2 text-lg font-bold leading-snug text-white sm:text-xl">{decodeHtmlEntities(active.title)}</p>
            </div>
          </a>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-0.5">
          {strip.map((item, i) => (
            <button
              ...
              aria-label={`Tampilkan ${decodeHtmlEntities(item.title)}`}
