  ...
  return (
    <div className="max-w-xs rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-lg dark:border-slate-700 dark:bg-slate-900">
      ...
      <p className="mt-1 text-lg font-bold" style={{ color: item.fill }}>
        {item.count ? `${item.count} topik` : item.traffic}
      </p>
      <p className="mt-0.5 text-[11px] text-slate-400 dark:text-slate-500">
        {item.count ? "digabung ke Lainnya" : "pencarian"}
      </p>
    </div>
  );
}
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-slate-900 dark:text-slate-100">Pencarian Trending (Google Trends)</h2>
            <p className="text-xs text-slate-400 dark:text-slate-500">
              Top pencarian Google Trends di {geo} &middot; {period} terakhir
            </p>
          </div>
        </div>
        {updatedAt && (
          <span className="shrink-0 text-[11px] text-slate-400 dark:text-slate-500">Diperbarui {formatRelativeTime(updatedAt)}</span>
        )}
      </div>

      {pieData.length === 0 ? (
        <p className="py-8 text-center text-sm text-slate-400 dark:text-slate-500">Belum ada data trending</p>
      ) : (
        <div className="h-72">
