...
import type { VideoMetadataDetail } from "@/features/youtube/hooks/useVideoSearch";
import {
  Eye,
  ExternalLink,
  Flame,
...
interface Props {
  open: boolean;
  onClose: () => void;
  detail: VideoMetadataDetail | null;
  loading: boolean;
  error: string;
}

export default function VideoDetailModal({ open, onClose, detail, loading, error }: Props) {
  const { t } = useTranslation();

  return (
    <Dialog open={open} onOpenChange={(next) => { if (!next) onClose(); }}>
      <DialogContent className="scrollbar-thin max-h-[90vh] w-full overflow-y-auto sm:max-w-4xl">
        {loading && (
          ...
          <>
            <div className="flex flex-col gap-4 sm:flex-row">
              ...
              <DialogHeader className="flex-1">
                <DialogTitle className="leading-snug">{decodeHtmlEntities(detail.title)}</DialogTitle>
                <a
                  href={detail.url}
                  target="_blank"
