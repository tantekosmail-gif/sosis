...
export default function ExecutiveInsightCard({ topic }: { topic: ExecutiveInsightTopic }) {
  ...
  const sentimentClause = dominant
    ? ei.fallbackSentimentClause
        .replace("{sentiment}", SENTIMENT_LABEL[dominant])
        .replace("{pct}", dominantPct.toFixed(1))
    : "";

  const headline = data?.headline || ei.fallbackHeadline.replace("{topic}", topic.topic);
  const insight =
    data?.insight ||
    ei.fallbackInsight
      .replace("{score}", topic.score.toFixed(1))
      .replace("{count}", String(topic.posts.length))
      .replace("{sentimentClause}", sentimentClause);
  ...
  return (
    <div className="flex h-full flex-col justify-between rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-sm">
      <div>
        <div className="flex items-center justify-between gap-3">
          ...
          >
            ...
            {loading ? ei.generating : data ? ei.regenerate : ei.generate}
          </button>
        </div>
      ...
      <div className="mt-5 flex items-center gap-6 border-t border-slate-800 pt-4">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">{ei.scoreLabel}</p>
          <p className={`${jetBrainsMono.className} mt-0.5 text-lg font-medium text-white`}>{topic.score.toFixed(1)}</p>
        </div>
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">{ei.positiveSentimentLabel}</p>
