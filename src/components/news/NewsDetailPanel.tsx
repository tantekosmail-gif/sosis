...
import type { NewsMetadataDetail } from "@/features/news/hooks/useNewsMetadataSearch";
import {
  Eye,
  ExternalLink,
  Flame,
...
const SCORE_ITEMS: { key: keyof NewsMetadataDetail["scores"]; icon: typeof Flame; labelKey: "trendScoreLabel" | "engagementScoreLabel" | "freshnessScoreLabel" | "authorityScoreLabel" }[] = [
...
interface Props {
  detail: NewsMetadataDetail | null;
  loading: boolean;
  error: string;
}

export default function NewsDetailPanel({ detail, loading, error }: Props) {
    ...
    <div className="space-y-4">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            {decodeHtmlEntities(detail.author || "Unknown")} <ExternalLink size={11} />
          </a>
        </div>
      </div>
      ...
      {detail.metrics && (
        <div className="flex flex-wrap items-center gap-4 text-sm font-semibold text-slate-700 dark:text-slate-300">
          <span className="flex items-center gap-1.5">
            <Eye size={14} className="text-slate-400 dark:text-slate-500" /> {formatCompact(detail.metrics.views ?? 0)}
          </span>
          <span className="flex items-center gap-1.5">
            <ThumbsUp size={13} className="text-slate-400 dark:text-slate-500" /> {formatCompact(detail.metrics.likes ?? 0)}
          </span>
          {(detail.saved_comments_count ?? 0) > 0 && (
            <span className="flex items-center gap-1.5">
              <MessageCircle size={13} className="text-slate-400 dark:text-slate-500" /> {formatCompact(detail.saved_comments_count ?? 0)}
            </span>
          )}
      ...
      <div className="border-t border-slate-100 pt-4 dark:border-slate-800">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">
          {t.videoDetailModal.commentsLabel} ({detail.saved_comments_count})
        </p>
