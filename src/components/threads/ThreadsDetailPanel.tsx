...
import type { ThreadsMetadataDetail } from "@/features/threads/hooks/useThreadsMetadataSearch";
import {
  ExternalLink,
  Flame,
  Loader2,
...
const SCORE_ITEMS: { key: keyof ThreadsMetadataDetail["scores"]; icon: typeof Flame; labelKey: string }[] = [
...
interface Props {
  detail: ThreadsMetadataDetail | null;
  loading: boolean;
  error: string;
}

export default function MetaDetailPanel({ detail, loading, error }: Props) {
    ...
    <div className="space-y-4">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="line-clamp-3 text-sm font-semibold leading-snug text-slate-900 dark:text-slate-100">
            {decodeHtmlEntities(detail.title || detail.content)}
          </h2>
          <a href={detail.url} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex w-fit items-center gap-1 text-xs text-slate-500 hover:text-indigo-600 dark:text-slate-400">
      ...
      <div className="grid grid-cols-4 gap-2">
        {SCORE_ITEMS.map(({ key, icon: Icon, labelKey }) => (
          <div key={key} className="flex flex-col items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 py-2.5 dark:border-slate-700 dark:bg-slate-800">
            <Icon size={14} className="text-indigo-500" />
            <span className="text-sm font-bold text-slate-800 dark:text-slate-200">{formatScore(detail.scores[key])}</span>
            <span className="text-center text-[10px] leading-tight text-slate-400 dark:text-slate-500">{t.videoDetailModal[labelKey]}</span>
          </div>
        ))}
      </div>
      <div className="flex flex-wrap items-center gap-4 text-sm font-semibold text-slate-700 dark:text-slate-300">
        <span className="flex items-center gap-1.5">
          <ThumbsUp size={14} className="text-slate-400 dark:text-slate-500" /> {formatCompact(detail.metrics.likes)}
        </span>
        ...
        {detail.metrics.quotes > 0 && (
          <span className="flex items-center gap-1.5">
            <Quote size={13} className="text-slate-400 dark:text-slate-500" /> {formatCompact(detail.metrics.quotes)}
          </span>
        )}

        {detail.metrics.author_followers > 0 && (
          <span className="flex items-center gap-1.5">
            <ThumbsUp size={13} className="text-slate-400 dark:text-slate-500" /> {formatCompact(detail.metrics.author_followers)}
          </span>
        )}
      ...
      {detail.ai_summary && (
        <div className="rounded-xl border border-indigo-100 bg-indigo-50/60 p-3.5 dark:border-indigo-900 dark:bg-indigo-950/30">
          <p className="text-xs font-semibold uppercase tracking-wide text-indigo-500 dark:text-indigo-400">{t.videoDetailModal.aiSummaryLabel}</p>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {detail.ai_tags.map((tag) => (
              <span key={tag} className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">{tag}</span>
            ))}
          </div>
        </div>
      )}
        ...
        <div className="border-t border-slate-100 pt-4 dark:border-slate-800">
          ...
          <div className="flex flex-wrap gap-1.5">
            {(detail.source_topics?.length ? detail.source_topics : [detail.source_topic]).map((kw) => (
              <span key={kw} className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300">{kw}</span>
            ))}
          </div>
        </div>
      )}
          ...
          <ul className="scrollbar-thin max-h-72 divide-y divide-slate-100 overflow-y-auto rounded-xl border border-slate-200 bg-white dark:divide-slate-800 dark:border-slate-700 dark:bg-slate-900">
            {detail.comments.map((comment, i) => (
              <li key={i} className="px-4 py-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-1.5">
                    ...
                    {formatRelativeTime(comment.published_at) && (
                      <span className="shrink-0 text-[11px] font-normal text-slate-400 dark:text-slate-500">· {formatRelativeTime(comment.published_at)}</span>
                    )}
                  </div>
                ...
                <p className="mt-1 whitespace-pre-wrap break-words text-sm leading-snug text-slate-600 dark:text-slate-400">{decodeHtmlEntities(comment.content)}</p>
              </li>
            ))}
          </ul>
        )}
