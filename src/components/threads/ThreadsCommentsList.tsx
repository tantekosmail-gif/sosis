...
import type { ThreadsComment } from "@/features/threads/types/search.types";
...
export default function ThreadsCommentsList({
  data,
  totalInDb,
  onRefresh,
}: {
  ...
  const hasMore = typeof totalInDb === "number" && totalInDb > data.length;
  ...
  if (!data || data.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-white dark:bg-slate-900 py-12 text-center text-sm text-slate-400 dark:text-slate-500">
        Tidak ada balasan
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {hasMore && onRefresh && (
        <div className="flex items-center justify-between rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-2.5 text-xs text-slate-500 dark:text-slate-400">
          <span>
            Menampilkan {data.length} dari {totalInDb} balasan tersimpan
          </span>
          <button
            type="button"
            onClick={handleRefresh}
            disabled={refreshing}
            className="flex items-center gap-1.5 font-semibold text-purple-600 hover:text-purple-700 disabled:opacity-50"
          >
            ...
            <li key={comment.id ?? `${comment.author}-${idx}`} className="px-5 py-4">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                  @{comment.author}
                  {comment.reply_to && (
                    <span className="ml-1 font-normal text-slate-400 dark:text-slate-500">→ @{comment.reply_to}</span>
                  )}
                </span>
