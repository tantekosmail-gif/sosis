...
import { Download, Eye, FileText, PieChart, Reply, Users } from "lucide-react";

import type { TwitterAccountSummary, TwitterAnalysisSummary } from "@/features/twitter/types/summary.types";
...
function AccountAvatar({ username }: { username: string }) {
  return (
    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-sky-100 dark:bg-sky-950/50 text-sm font-bold text-sky-700 dark:text-sky-300">
      {username.charAt(0).toUpperCase()}
    </span>
  );
}

function AccountSentimentBar({ sentiment }: { sentiment: TwitterAccountSummary["sentiment"] }) {
...
function downloadAccountsCsv(rows: TwitterAccountSummary[]) {
  const header = ["Username", "Posts", "Balasan", "Positif (%)", "Netral (%)", "Negatif (%)"];
  ...
  a.href = url;
  a.download = "analisis-twitter-per-akun.csv";
  a.click();
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatCard icon={FileText} label="Total Post" value={overall.total_posts.toLocaleString("id-ID")} />
        <StatCard icon={Reply} label="Total Balasan" value={overall.total_comments.toLocaleString("id-ID")} />
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-400 dark:text-slate-500">Agregat sentimen dari seluruh balasan yang dianalisis.</p>
            </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-950 text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right">Balasan</th>
