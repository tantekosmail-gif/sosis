...
import { BadgeCheck, Feather, Info, MessageSquare, Percent, PieChart, TrendingUp } from "lucide-react";

import type { TwitterPageInfo, TwitterPostItem, TwitterPostsStats, TwitterSentimentOverview } from "@/features/twitter/types/posts.types";
...
export default function TwitterProfileCard({
  ...
  const engagementRate = (() => {
    ...
    const totalEngagement = items.reduce(
      (sum, item) => sum + (item.likes || 0) + (item.comment_count || 0) + (item.retweet_count || 0) + (item.quote_count || 0),
      0
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        <StatCard icon={Feather} label="Total Tweet" value={stats.total_posts.toLocaleString("id-ID")} />
        <StatCard icon={MessageSquare} label="Total Balasan" value={stats.total_comments.toLocaleString("id-ID")} />
        <StatCard
          icon={TrendingUp}
          label="Dianalisis"
          value={stats.total_analyzed.toLocaleString("id-ID")}
          tooltip="Jumlah balasan yang sudah diproses oleh mesin analisis sentimen"
        />
        <StatCard
          icon={PieChart}
          label="Coverage"
          value={`${stats.coverage_pct.toFixed(1)}%`}
          tooltip="Persentase balasan yang sudah dianalisis dari total balasan yang terkumpul"
        />
        <StatCard
          icon={Percent}
          label="Engagement Rate"
          value={engagementRate !== null ? `${engagementRate.toFixed(2)}%` : "-"}
          tooltip="Rata-rata (like + balasan + retweet + quote) per tweet dibagi jumlah followers, dari tweet yang diambil saat ini"
        />
      </div>
