  ...
  return (
    <div
      className={`rounded-xl border p-3 transition-colors ${
        isSelected ? "border-sky-400 bg-sky-50/40" : "border-slate-100 bg-white"
      }`}
    >
      <a
        href={post.url}
        ...
        className="flex items-start gap-1 text-xs font-medium leading-snug text-slate-700 hover:text-sky-600"
      >
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-start justify-between gap-4 p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {topic.twitter_identifier && <p className="text-xs text-slate-400">Sumber: @{topic.twitter_identifier}</p>}
          </div>
        </div>
      ...
      <div className="border-t border-slate-100 px-5 py-4">
        <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Tweet Terkait</p>
        {topic.posts.length === 0 ? (
          <p className="text-sm text-slate-400">Belum ada tweet untuk topik ini</p>
        ) : (
