...
import { useTikTokDiscover } from "@/features/tiktok/hooks/useTikTokDiscover";
...
function SubmissionBadge({ keyword, submitted }: { keyword: string; submitted: { created: string[]; updated: string[]; rejected: string[] } }) {
  ...
  if (submitted.updated.includes(keyword)) {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50 dark:bg-rose-950/40 px-2.5 py-1 text-[11px] font-semibold text-rose-700">
        <CheckCircle2 size={12} />
...
export default function TikTokDiscoverPanel({ onSelectAccount }: { onSelectAccount?: (username: string) => void }) {
  const [open, setOpen] = useState(false);
  const [keywordInput, setKeywordInput] = useState("");
  const { data, loading, error, discover } = useTikTokDiscover();
  ...
  return (
    <>
      <button
        ...
        className="flex h-10 shrink-0 items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 text-sm font-semibold text-rose-600 shadow-sm transition hover:bg-rose-50 dark:hover:bg-rose-950/40"
      >
        <Radar size={15} />
        Cari Topik & Akun
      </button>
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-xl">
          <DialogHeader>
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-rose-50 dark:bg-rose-950/40">
          ...
          <div className="space-y-5">
            <form onSubmit={handleSubmit} className="flex items-center gap-3">
              ...
              <button
                ...
                className="flex h-10 shrink-0 items-center gap-2 rounded-xl bg-rose-600 px-4 text-sm font-semibold text-white hover:bg-rose-700 disabled:opacity-50 transition"
              >
            ...
            {loading && (
              <div className="flex flex-col items-center justify-center py-10">
                <Loader2 size={24} className="mb-2 animate-spin text-rose-400" />
                <p className="text-sm text-slate-500 dark:text-slate-400">Mencari akun & video terkait...</p>
              </div>
            )}
              ...
              <div className="space-y-5">
                <div className="flex flex-wrap items-center gap-3">
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    <span className="font-semibold text-slate-700 dark:text-slate-300">{data.posts_found}</span> video ·{" "}
                    <span className="font-semibold text-slate-700 dark:text-slate-300">{data.accounts_found.length}</span> akun ditemukan untuk{" "}
                    <span className="font-semibold text-rose-600">&quot;{data.keyword}&quot;</span>
                  </p>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {data.accounts_found.map((account) => (
                        <button
                          ...
                          className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-2.5 py-1 text-xs font-medium text-slate-600 dark:text-slate-400 transition hover:border-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950/40 hover:text-rose-700"
                        >
                          @{account.username}
                        </button>
                      ))}
                ...
                {data.sample_posts.length > 0 && (
                  <div>
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Contoh Video</p>
                    <ul className="space-y-2">
                      {data.sample_posts.map((post, idx) => (
                        <li key={`${post.identifier_extracted}-${idx}`} className="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3">
                          <p className="line-clamp-2 text-xs leading-snug text-slate-600 dark:text-slate-400">{post.caption}</p>
                          <div className="mt-1.5 flex items-center justify-between gap-2 text-[11px] text-slate-400 dark:text-slate-500">
                            ...
                            <a
                              ...
                              className="flex shrink-0 items-center gap-1 text-rose-500 hover:text-rose-700"
                            >
