...
function SubmissionBadge({ keyword, submitted }: { keyword: string; submitted: { created: string[]; updated: string[]; rejected: string[] } }) {
  ...
  if (submitted.updated.includes(keyword)) {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50 px-2.5 py-1 text-[11px] font-semibold text-rose-700">
        <CheckCircle2 size={12} />
      ...
      <div className="space-y-5 p-6">
        <form onSubmit={handleSubmit} className="flex items-center gap-3">
          <input
            ...
            className="h-10 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3.5 text-sm text-slate-800 placeholder:text-slate-400 focus:border-rose-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-rose-500/20 transition"
          />
          <button
            type="submit"
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-10">
            ...
            <p className="text-sm text-slate-500">Mencari akun & video terkait...</p>
          </div>
        )}
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-3">
              <p className="text-sm text-slate-500">
                <span className="font-semibold text-slate-700">{data.posts_found}</span> video ·{" "}
                ...
                <div className="flex flex-wrap gap-2">
                  {data.accounts_found.map((account) => (
                    <button
                      ...
                      className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-600 transition hover:border-rose-300 hover:bg-rose-50 hover:text-rose-700"
                    >
                      @{account.username}
                    </button>
                  ))}
            ...
            {data.sample_posts.length > 0 && (
              <div>
                <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Contoh Video</p>
                <ul className="space-y-2">
                  {data.sample_posts.map((post, idx) => (
                    <li key={`${post.identifier_extracted}-${idx}`} className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                      <p className="line-clamp-2 text-xs leading-snug text-slate-600">{post.caption}</p>
