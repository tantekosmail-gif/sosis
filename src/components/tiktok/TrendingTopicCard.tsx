...
import { Activity, AtSign, Eye, ExternalLink, Hash, MessageCircle, ThumbsUp } from "lucide-react";
...
function getDominantSentiment(sentiment: TikTokTrendingTopic["sentiment"]) {
  ...
  return (
    <div
      className={`rounded-xl border p-3 transition-colors ${
        isSelected ? "border-rose-400 bg-rose-50/40" : "border-transparent bg-slate-50 dark:bg-slate-950"
      }`}
    >
      <a
        href={post.url}
        ...
        className="flex items-start gap-1 text-xs font-medium leading-snug text-slate-700 dark:text-slate-300 hover:text-rose-600"
      >
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-2 px-5 pb-4">
        <StatBox label="Jumlah Video" value={String(topic.posts.length)} />
        <StatBox
          label="Sentimen"
      ...
      <div className="flex-1 border-t border-slate-100 dark:border-slate-800 px-5 py-4">
        <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Video Terkait</p>
        {topic.posts.length === 0 ? (
          <p className="text-sm text-slate-400 dark:text-slate-500">Belum ada video untuk topik ini</p>
        ) : (
          <div className="space-y-3">
            ...
            {extraCount > 0 && (
              <p className="text-[11px] text-slate-400 dark:text-slate-500">+{extraCount} video lainnya</p>
            )}
          </div>
        )}
      </div>
