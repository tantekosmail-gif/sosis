...
function extractTopHashtags(items: TikTokPostItem[]) {
...
export default function TopHashtags({ items }: { items: TikTokPostItem[] }) {
    ...
    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
      ...
      <div className="flex flex-wrap gap-2">
        {hashtags.map((h) => (
          <span
            key={h.tag}
            className="inline-flex items-center gap-1 rounded-lg bg-rose-50 dark:bg-rose-950/40 px-2.5 py-1 text-xs font-medium text-rose-700 dark:text-rose-300"
          >
            <Hash size={11} />
            {h.tag}
            <span className="text-rose-400 dark:text-rose-500">{h.count}</span>
          </span>
        ))}
      </div>
    </div>
