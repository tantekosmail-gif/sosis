...
function AccountAvatar({ username }: { username: string }) {
  return (
    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-950/50 text-sm font-bold text-blue-700 dark:text-blue-300">
      {username.charAt(0).toUpperCase()}
    </span>
  );
}

function AccountSentimentBar({ sentiment }: { sentiment: FacebookAccountSummary["sentiment"] }) {
...
function downloadAccountsCsv(rows: FacebookAccountSummary[]) {
  ...
  a.href = url;
  a.download = "analisis-facebook-per-akun.csv";
  a.click();
  URL.revokeObjectURL(url);
}

export default function FacebookSummaryWidget({
  data,
  onSelectAccount,
}: {
  data: FacebookAnalysisSummary;
